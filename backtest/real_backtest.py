"""
بکتست واقعی با دادههای Gate.io
"""
...
def run_backtest_for_coin(client, strategy, engine, symbol, timeframe='5m', limit=2000):
    """اجرای بکتست برای یک ارز"""
    print(f"\n{'='*60}")
    print(f"🔍 بکتست {symbol}")
    print(f"{'='*60}")
...
def main():
    print("🚀 شروع بکتست واقعی با دادههای Gate.io")
    print("=" * 60)
    ...
    for symbol in TRADING_COINS:
        try:
            engine = BacktestEngine(initial_capital=1000)
            result = run_backtest_for_coin(client, strategy, engine, symbol)
            if result:
    ...
    if all_results:
        ...
        print(f"\n📁 نتایج در backtest_results.csv ذخیره شد")
